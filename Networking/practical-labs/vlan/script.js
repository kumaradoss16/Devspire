document.addEventListener('DOMContentLoaded', () => {
  ...
  // Adds a small "#" link next to each content heading so readers can
  // copy a direct link to that section.
  ...
  // (security best practice) and marks them visually.
  ...
  // Pressing "/" focuses the search box, if one exists on the page.
  ...
  // Falls back to a placeholder image if any content image fails to load.
  ...
  // Dev-only sanity check: warns in console if <title>, og:title, canonical,
  // and the TechArticle JSON-LD headline don't all agree — catches the
  // copy-paste-leftover metadata bug that's shown up on several pages.
  ...
  // Floating "Share" button that opens a small popup menu with direct
  // links to WhatsApp, Telegram, Gmail, X/Twitter, Facebook, LinkedIn,
  // and a "Copy link" option — instead of relying on navigator.share(),
  // which on most desktop browsers only offers a copy-link fallback and
  // never shows these apps as options.
  function initShareButton() {
    ...
    // Rebuilds the menu each time it opens, so the share links always
    // reflect the current page's title/URL.
    function buildMenu() {
  ...
  // Run all page enhancements now — direct calls, since we're already
