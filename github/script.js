...
const CATS = {
  ...
  security: { label:'Security',     emoji:'🔐', desc:'Hacking & pen-testing',  color:'#22d3a0', dim:'rgba(34,211,160,.09)',  border:'rgba(34,211,160,.28)'  },
...
const LANGCOLORS = {
  ...
  'Java':'#b07219','Kotlin':'#A97BFF','Shell':'#89e051','HTML':'#e34c26',
  'CSS':'#563d7c','Ruby':'#701516','Swift':'#F05138','PHP':'#4F5D95',
  'Elixir':'#6e4a7e','Zig':'#ec915c','Nushell':'#4E9A06',
  'Jupyter Notebook':'#DA5B0B','PLpgSQL':'#336791',
  'Objective-C':'#438eff','PowerShell':'#012456','default':'#8892a4',
};
...
function buildCard(r) {
  ...
  return `<a href="${r.url}" target="_blank" rel="noopener noreferrer"
    ...
    ${topics.length
      ? `<div class="cardtopics">${topics.map(t => `<span class="cardtopic">${t}</span>`).join('')}</div>`
      : ''}
...
function buildMenu() {
  ...
  allCats.forEach((cat, i) => {
    ...
    const count = cat === 'all' ? REPOS.length : REPOS.filter(r => r.cat === cat).length;
    const isSel = cat === active;
    if (i > 0) html += '<div class="sbmenu-div"></div>';
    html += `<button class="sbmenu-item${isSel ? ' selected' : ''}"
      data-cat="${cat}"
  ...
  menu.querySelectorAll('.sbmenu-item').forEach(btn => {
    btn.addEventListener('click', function(e) {
      ...
      renderGrid();
      updateBanner();
      menu.querySelectorAll('.sbmenu-item').forEach(function(b) {
        var sel = b.dataset.cat === active;
...
function openMenu() {
  menuOpen = true;
  var m = document.getElementById('sb-menu');
  var t = document.getElementById('sb-trigger');
...
/* ══════════════════════════════════════════════════════════
   DOM READY — ALL init calls are inside here
══════════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (el) {
    var text         = el.dataset.text || 'DevSpireHub';
    var i            = 0;
    var isDeleting   = false;
    var typeSpeed    = 100;
    var deleteSpeed  = 50;
    var pauseTime    = 2000;
    var restartPause = 500;
    function typeLoop() {
  ...
  if (scrollBtn) {
    var circle        = scrollBtn.querySelector('.progress-circle');
